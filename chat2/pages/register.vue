...
<div class="m-4 p-4 bg-white shadow rounded">
  ...
  <form @submit.prevent="onSubmit">
    ...
    <div class="flex items-center justify-center flex-col w-full h-full mt-8">
      <div :class="{'border-red-500': form.imageUrl.errorMessage}" class="flex items-center justify-center w-32 h-32 bg-gray-200 rounded-full border border-gray-400 ">
        ...
        <input ref="image" type="file" style="display: none" accept="image/*" @change="onSelectFile" />
      </div>
    ...
    <label class="block mt-8 mb-2 ml-2 uppercase tracking-wide text-darkGray text-s">
      名前
    </label>
    <div class="h-20 mb-6">
      <input v-model="form.name.val" :class="{'border-red-500': form.name.errorMessage}" type="text" class="block w-full py-3 px-4 appearance-none bg-gray-200 text-darkGray border rounded leading-tight focus:outline-none focus:bg-white" @blue="validateName" />
