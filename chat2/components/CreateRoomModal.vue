...
<div class="bg-white shadow rounded-lg m-4 w-11/12 p-4">
  <form @submit.prevent="onSubmit">
    <div class="flex justify-center items-center flex-col mt-3">
      <div :class="{ 'border border-red-500': form.imageUrl.errorMessage }" class="rounded-lg w-32 h-32 bg-gray-200 flex items-center justify-center">
        <template v-if="form.imageUrl.val">
          <img :src="form.imageUrl.val" @click="selectImage" class="w-32 h-32 object-cover border rounded-lg" />
        </template>
        <template v-else>
        ...
        <input ref="image" @change="onFileSelect" type="file" style="display: none" accept="image/*" />
      </div>
      <span v-show="form.imageUrl.errorMessage" class="text-red text-sm text-center">
        {{ form.imageUrl.errorMessage }}
      </span>
    </div>
    <div class="h-20 mb-4 mt-10">
      <textarea v-model="form.name.val" @blur="validateName" :class="{ 'border-red-500': form.name.errorMessage }" placeholder="部屋の名前を入力しよう！" class="appearance-none block w-full bg-gray-200 text-darkGray border rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white" maxlength="mexLength" />
